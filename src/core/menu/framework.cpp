...
void gui::framework::begin( sdk::vec2_t pos, sdk::vec2_t size ) {
	...
	photon->portal2->surface->enable_clipping = true;
	photon->portal2->surface->set_clip_rect( cur_menu.pos.x + 1, cur_menu.pos.y + 1, cur_menu.pos.x + cur_menu.size.x - 1, cur_menu.pos.y + cur_menu.size.y - 1 );
}
...
void gui::framework::end( ) {
	// draw dropdowns
	{
		...
		for ( std::size_t i = 0; i < cur_dropdown.items.size( ); ++i ) {
			...
			bool clicking = hover && photon->input->get_key_press( sdk::mouse_left );
...
bool gui::framework::button( sdk::vec2_t size, const std::string &label ) {
	...
	if ( hover && photon->input->get_key_press( sdk::mouse_left ) ) return true;
...
void gui::framework::slider( int &val, int min, int max, const std::string &label ) {
	...
	photon->render->draw_text( cur_pos.x + size.x + 4, cur_pos.y - 3, fonts::normal, colors::white, false, utils::string::ssprintf( "%d", val ).c_str( ) );
...
void gui::framework::sliderf( float &val, float min, float max, const std::string &label ) {
	...
	photon->render->draw_text( cur_pos.x + size.x + 4, cur_pos.y - 3, fonts::normal, colors::white, false, utils::string::ssprintf( "%.1f", val ).c_str( ) );
...
void gui::framework::multicombo( std::size_t &val, std::vector<std::string> items, const std::string &label ) {
	...
	photon->portal2->surface->set_clip_rect( cur_pos.x + 8, cur_pos.y, cur_pos.x + size.x - 8, cur_pos.y + size.y );
