...
void c_input::poll_input( ) {
	...
	for ( std::size_t i = 0; i < 256; ++i ) {
		cur_keys[ i ] = photon->portal2->input_system->is_button_down( ( sdk::button_code_t ) i );
	}

	photon->portal2->input_system->get_cursor_position( &cursor_x, &cursor_y );
}
...
sdk::vec2_t c_input::get_cursor_position( ) {
	return sdk::vec2_t( cursor_x, cursor_y );
}
